...
Análisis de las características más interesantes de gestión financiera que ofrecen las aplicaciones de algunos de los bancos españoles más grandes\cite{articulo-bancos}:
...
\begin{itemize}
    ...
    En la aplicación ImaginBank, con el servicio \textit{MyMonz}\cite{MyMonz} se recibe un informe mensual de gastos e ingresos en la cuenta que podemos ver por categorías. 
    ...
    En cuanto a los planes de ahorro, en el apartado \textit{Mi Hucha}\cite{hucha-imagin} (Figura \ref{fig:hucha_objetivo_imagin}) podemos crear 
    ...
    La aplicación del Banco Santander\cite{Santander}
    ...
    Con \textit{Mi día a día}\cite{dia-dia}  
    obtenemos, al igual que en el resto de aplicaciones, el resumen con los movimientos de la cuenta.
    Respecto a los métodos para ahorrar existe una cuenta gratuita denominada cuenta metas\cite{metas}
    en la que apartar dinero a modo de hucha para conseguir hasta un máximo de 5 objetivos. 

    El apartado Presupuestos\cite{presupuestos-BBVA} es el lugar donde definir una cantidad máxima de gasto deseada 
    ...
    Encontramos también Apartados\cite{Apartados} donde el objetivo es el mismo que en presupuestos, con la diferencia de 
